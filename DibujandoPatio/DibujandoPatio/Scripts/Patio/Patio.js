...
async function generarPreview(area, containerId) {
    ...
    const LienzoPreview = new Lienzo({
        Stage: stage,
        Layer: layer,
        Escala: DameEscala(),
    ...
    area.Vertices.forEach(v => {
        LienzoPreview.PuntoActual = LienzoPreview.AgregarPunto(v.X, v.Y);
        LienzoPreview.PuntoActual.Id = v.Id;
        LienzoPreview.PuntoActual.Dibujar(LienzoPreview, LienzoPreview.Layer, LienzoPreview.Modo, LienzoPreview.Estado);
    });
    ...
    const scaleX = 150 / stage.width();
    const scaleY = 150 / stage.height();
    const scale = Math.min(scaleX, scaleY);
    stage.scale({ x: scale, y: scale });
    stage.draw();

    const dataUrl = stage.toDataURL({ pixelRatio: 0.9 });
    stage.destroy();
